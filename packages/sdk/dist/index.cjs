...
const PROGRAM_IDS = {
    ...
    DARK_PROTOCOL: new web3_js.PublicKey('E8zL7h9qHjC7sMf2WCYhdqS5iLkYhPJ9yAhTfevo74jm'),
    /** Alias for the shielded wallet program (same contract) */
    SHIELDED_WALLET: new web3_js.PublicKey('E8zL7h9qHjC7sMf2WCYhdqS5iLkYhPJ9yAhTfevo74jm'),
    /** MAINNET placeholder — replace after `solana program deploy` on mainnet */
    DARK_PROTOCOL_MAINNET: new web3_js.PublicKey('E8zL7h9qHjC7sMf2WCYhdqS5iLkYhPJ9yAhTfevo74jm'),
};
/**
...
class DarkProtocolClient {
    ...
    static async create(config) {
        ...
        const program = new anchor.Program(DARK_PROTOCOL_IDL, provider);
     ...
     */
    protocolStatePDA() {
        return web3_js.PublicKey.findProgramAddressSync([Buffer.from(PROTOCOL_STATE_SEED)], this.programId);
    }
    /**
    ...
    poolVaultPDA() {
        ...
        return web3_js.PublicKey.findProgramAddressSync([Buffer.from(POOL_VAULT_SEED), statePDA.toBuffer()], this.programId);
    }
    /**
     * Return the PDA for a shielded note given its commitment.
     */
    notePDA(commitment) {
        return web3_js.PublicKey.findProgramAddressSync([Buffer.from('note'), commitment], this.programId);
    }
    /**
     * Return the PDA for a nullifier record.
     */
    nullifierPDA(nullifier) {
        return web3_js.PublicKey.findProgramAddressSync([Buffer.from('nullifier'), nullifier], this.programId);
    }
    /**
    ...
    async getProtocolState() {
        ...
        try {
            ...
            const authority = new web3_js.PublicKey(raw.data.slice(8, 40));
    ...
    async getNote(commitment) {
        ...
        try {
            ...
            const dep = new web3_js.PublicKey(raw.data.slice(off + 32 + 580 + 80 + 32 + 8 + 1 + 8 + 8, off + 32 + 580 + 80 + 32 + 8 + 1 + 8 + 8 + 32));
...
class DarkWallet {
    ...
    static async fromMnemonic(client, mnemonic, _accountIndex = 0) {
        ...
        const keypair = web3_js.Keypair.fromSeed(seed.slice(0, 32));
    ...
    static fromPrivateKey(client, privateKey) {
        return new DarkWallet(client, web3_js.Keypair.fromSecretKey(privateKey));
    }
    /**
     * Generate new wallet
     */
    ...
    root() {
        ...
        while (level.length > 1) {
            ...
            for (let i = 0; i < level.length; i += 2) {
                ...
                next.push(sha256.sha256(new Uint8Array([...left, ...right])));
            }
            level = next;
        }
        return level[0];
    ...
    witness(index) {
        ...
        while (level.length > 1) {
            ...
            for (let i = 0; i < level.length; i += 2) {
                const l = level[i];
                const r = level[i + 1] ?? l;
                next.push(sha256.sha256(new Uint8Array([...l, ...r])));
            }
            level = next;
...
class ShieldedWallet {
    ...
    getDiversifiedAddress(index) {
        ...
        const indexBytes = new Uint8Array(4);
        new DataView(indexBytes.buffer).setUint32(0, index, true);
        const diversifier = blake3.blake3(new Uint8Array([...this.ivk.ivk, ...indexBytes, ...Buffer.from('div', 'utf8')]), { dkLen: 11 });
        diversifier[0] = diversifier[0] || 1;
    ...
    createPaymentProof(params) {
        ...
        const sig = Buffer.from(sha256.sha256(new Uint8Array([...Buffer.from(fields, 'utf8'), ...this.fvk.ovk]))).toString('hex');
        return {
            version: 1,
            noteId: note.id,
            commitment: note.commitment,
    ...
    static verifyPaymentProof(proof, ovk) {
        try {
            ...
            if (ovk) {
                const expected = Buffer.from(sha256.sha256(new Uint8Array([...Buffer.from(fields, 'utf8'), ...ovk]))).toString('hex');
    ...
    _storeNote(encNote, value, memo, recipient, slot = 0) {
        ...
        const rseedHex = Buffer.from(sha256.sha256(new Uint8Array([...encNote.cm, ...encNote.epk]))).toString('hex');
        const note = {
            id: commitmentHex,
    ...
    async _submitDeposit(note, encNote) {
        ...
        const client = await DarkProtocolClient.create({
            ...
            programId: new web3_js.PublicKey(this.programId),
        });
    ...
    async _submitTransfer(inputNote, outputNote, encNote) {
        ...
        const outCommitment2 = Uint8Array.from(sha256.sha256(new Uint8Array([...outCommitment1, 0x01])));
        ...
        epk2.set(sha256.sha256(encNote.epk).slice(0, 32));
        const [statePDA] = client.protocolStatePDA();
...
class MultisigShieldedWallet extends ShieldedWallet {
    ...
    proposeTransfer(params) {
        const id = Buffer.from(sha256.sha256(new Uint8Array([
...
exports.CommitmentTree = CommitmentTree;
...
exports.MultisigShieldedWallet = MultisigShieldedWallet;
exports.NOTEENCRYPTION_AUTH_BYTES = NOTEENCRYPTION_AUTH_BYTES;
exports.NOTE_SEED = NOTE_SEED;
exports.NULLIFIER_SEED = NULLIFIER_SEED;
...
exports.POOL_VAULT_SEED = POOL_VAULT_SEED;
exports.PROGRAM_IDS = PROGRAM_IDS;
exports.PROTOCOL_STATE_SEED = PROTOCOL_STATE_SEED;
...
exports.SHIELDED_WALLET_PROGRAM_ID = SHIELDED_WALLET_PROGRAM_ID;
...
exports.ShieldedWallet = ShieldedWallet;
