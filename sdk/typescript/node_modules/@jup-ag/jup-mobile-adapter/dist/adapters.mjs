...
// src/adapters/Reown/useWrappedReownAdapter.tsx
...
// src/adapters/Reown/CustomReownAdapter.tsx
import {
...
var REOWN_WALLET_NAME = "WalletConnect/Reown";
var CustomReownAdapter = class _CustomReownAdapter extends BaseMessageSignerWalletAdapter {
  constructor(config) {
    super();
    this.name = REOWN_WALLET_NAME;
    this.url = "https://reown.com";
    ...
    this._accountChanged = (_newPublicKey) => {
  ...
  static setWalletProvider(walletProvider) {
    _CustomReownAdapter.walletProvider = walletProvider;
  }
  get publicKey() {
  ...
  connect() {
    return __async(this, null, function* () {
      try {
        ...
        const publicKey = yield this._config.open();
        if (publicKey) {
    ...
    return __async(this, null, function* () {
      ...
      if (wallet) {
        ...
        try {
          ...
          yield this._config.disconnectInternal();
        } catch (error) {
  ...
  sendTransaction(_0, _1) {
    return __async(this, arguments, function* (transaction, connection, options = {}) {
      try {
        const walletProvider = _CustomReownAdapter.walletProvider;
        ...
        try {
          ...
          const signedTx = yield walletProvider.signTransaction(transaction);
  ...
  signMessage(message) {
    return __async(this, null, function* () {
      try {
        ...
        if (!walletProvider) throw new WalletNotConnectedError();
        try {
          const signature = yield walletProvider.signMessage(message);
          return signature;
        } catch (error) {
...
function initReownAppKit({
  ...
  const appKit = createAppKit(__spreadValues({
    adapters: [solanaWeb3JsAdapter],
    networks: [solanaNetwork]
  }, appKitOptions));
  return { solanaWeb3JsAdapter, appKit };
}
var useWrappedReownAdapter = ({
  appKitOptions
}) => {
  useMemo(() => {
    initReownAppKit({
  ...
  const connect = () => __async(void 0, null, function* () {
    ...
    const connectLoop = () => __async(void 0, null, function* () {
      yield new Promise((resolve) => setTimeout(resolve, 500));
      const walletProvider2 = CustomReownAdapter.walletProvider;
      const publicKey = walletProvider2 == null ? void 0 : walletProvider2.publicKey;
      if (publicKey) {
        reownAppKit.close();
        CustomReownAdapter.setWalletProvider(walletProvider2);
        return publicKey;
      }
    ...
    return yield connectLoop();
  });
  const { reownAdapter, jupiterAdapter } = useMemo(() => {
    const reownAdapter2 = new CustomReownAdapter({
      open: connect,
      close: reownAppKit.close,
      disconnectInternal: () => __async(void 0, null, function* () {
        yield disconnectInternal();
        CustomReownAdapter.setWalletProvider(null);
      })
    });
    const jupiterMobileProxyAdapter = new Proxy(reownAdapter2, {
        ...
        switch (prop) {
          ...
          case "connect":
            return () => __async(void 0, null, function* () {
              ...
              return reownAdapter2.connect();
            });
          case "disconnect":
            return () => __async(void 0, null, function* () {
              yield reownAdapter2.disconnect();
            });
          default: {
    ...
    return { reownAdapter: reownAdapter2, jupiterAdapter: jupiterMobileProxyAdapter };
  }, []);
  return {
    reownAdapter,
    jupiterAdapter
